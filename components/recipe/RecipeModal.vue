<template>
		...
		<div class="p-6 border-t">
		  ...
		  <div class="flex flex-col space-y-4">
			...
			<button
			  ...
			  :disabled="!selectedFolderId || rating === 0 || bookmarkLoading"
			  @click="bookmarkRecipe"
			>
			  {{ bookmarkLoading ? 'Bookmarking...' : 'Bookmark Recipe' }}
			</button>
		  </div>
		</div>
	  </div>
  ...
  const shouldShowToggle = computed(() => {
	const desc = selectedRecipe.value.description || '';
	return desc.length > maxDescriptionChars;
  });
  const toggleDescription = () => {
	descriptionExpanded.value = !descriptionExpanded.value;
  };
  // --- End Description Read More Toggle ---
  ...
  watch(() => selectedRecipe.value, () => {
	if (selectedRecipe.value) folderStore.fetchFolders();
  });
  </script>
