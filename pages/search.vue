<template>
	<div class="container mx-auto p-4">
		...
		<div
			v-else-if="hasSearched && !loading && !error && results.length === 0"
		...
		<div
			...
			class="text-gray-500 text-center py-4"
		>
			<p>Start searching to find delicious recipes!</p>
		</div>
...
const handlePageChange = async (page) => {
	hasSearched.value = true; // Pagination implies a search has occurred
	await router.push({
		path: '/search',
...
watch(
	...
	async ([newQuery, newPage]) => {
		...
		if (newQuery) {
			hasSearched.value = true; // URL with query implies a search
...
// Clear search store on mount if no query
onMounted(() => {
	if (!route.query.q) {
		searchStore.clearSearch();
		hasSearched.value = false; // Reset on initial load with no query
	}
});
</script>
